## Testing matplotlib on the Neural Network
...
## For this I add to each init function a parameter stat which has the main stats on it
## and then cycle through every Layer and print it's stats
...
class Model:
	...
	def stats(self, sigma, path_name=None):
		...
		f_ax1.set_title('Model structur:')
		layer_name = []
		for i in range(le):
		...
		# Save the figure
		path_name_png = str(path_name) + '.PNG'
		...
		with open(str(path_name), 'wb') as f:
...
print(X.shape[1])
# Add layers
model.add(Layer_Dense(X.shape[1], 12))
model.add(Activation_ReLU())
model.add(Layer_Dense(12, 12))
model.add(Activation_ReLU())
model.add(Layer_Dense(12, 10))
...
print(prediction)
model.stats(10, 'Optimizing_RPS/test')
